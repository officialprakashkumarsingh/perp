class APIHandler {
    ...
    async fetchPageContent(url) {
        ...
        if (match && match[1]) {
            try {
                const transcript = await this.getYouTubeTranscript(match[1]);
                if (transcript) {
                    return `Title: YouTube Video\nTranscript:\n${transcript}`;
                }
            } catch (e) {
                console.error("YouTube transcript fetch failed, falling back to page text", e);
            }
        }
    ...
    async getYouTubeTranscript(videoId) {
        try {
            // Use Piped API to get video metadata and subtitles
            ...
            if (!data.subtitles || data.subtitles.length === 0) {
                return null; // No subtitles
            }

            // Prefer English, then auto-generated English
            const sub = data.subtitles.find(s => s.code === 'en' && !s.autoGenerated) ||
                        data.subtitles.find(s => s.code === 'en') ||
                        data.subtitles[0];
            ...
            if (!subResponse.ok) throw new Error("Subtitle fetch error");
            const subText = await subResponse.text();

            // Parse VTT/XML/JSON based on format. Piped often returns VTT or JSON.
            // Simple VTT cleaner
            if (sub.mimeType.includes('vtt') || subText.includes('WEBVTT')) {
                return subText
                    .replace(/WEBVTT/g, '')
                    .replace(/NOTE .*/g, '')
                    ...
                    .replace(/\n+/g, ' ') // Join lines
                    .trim();
            } else if (sub.mimeType.includes('json')) {
                const json = JSON.parse(subText);
                if (Array.isArray(json)) {
                    // Piped sometimes returns standard JSON array of events
                    return json.map(item => {
                        return (item.segs || []).map(s => s.utf8).join('');
                    }).join(' ');
                } else if (json.events) {
                    return json.events.map(e => (e.segs || []).map(s => s.utf8).join('')).join(' ');
                }
            } else {
                 // XML (ttml) or other: simple tag strip
                 return subText.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
            }
